...
func (f *LLMTestFixture) Setup(ctx context.Context, k8sClient client.Client) (*acp.LLM, *corev1.Secret, error) {
	// Create secret first
...
func (f *LLMTestFixture) addProviderConfig(llm *acp.LLM) {
	switch f.provider {
	case "openai":
		llm.Spec.OpenAI = &acp.OpenAIConfig{
			Organization: "test-org",
			APIType:      "OPEN_AI",
		}
	case "anthropic":
		llm.Spec.Anthropic = &acp.AnthropicConfig{
	...
	case "mistral":
		...
		timeout := 30
		randomSeed := 42
		llm.Spec.Mistral = &acp.MistralConfig{
			MaxRetries: &maxRetries,
			Timeout:    &timeout,
			RandomSeed: &randomSeed,
		}
	case "google":
		llm.Spec.Google = &acp.GoogleConfig{
			CloudProject:  "test-project",
			CloudLocation: "us-central1",
		}
	case "vertex":
		llm.Spec.Vertex = &acp.VertexConfig{
			CloudProject:  "test-project",
			CloudLocation: "us-central1",
		}
	}
...
func (f *LLMTestFixture) SetupWithoutAPIKey(ctx context.Context, k8sClient client.Client) (*acp.LLM, error) {
...
func (f *LLMTestFixture) SetupWithoutProviderConfig(ctx context.Context, k8sClient client.Client) (*acp.LLM, error) {
	// Create secret first
	secret := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{
			Name:      f.secretName,
